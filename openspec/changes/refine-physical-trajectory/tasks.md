...
continuous safety. This initial prerequisite did not include a scientific report
or executor. Deadline checks are cooperative, not native-call preemption.
...
D4.1 report implementation (based on `db532de`): separate immutable nominal/tighter
records enforce complete versus absent trajectory data and fixed-command replay.
Residuals and numerical agreement derive from the stored states, with explicit
comparison-failure/unavailable reasons. Finite detached JSON separates scientific
data from wall time and always disclaims continuous safety. 137 focused checks
pass, including budget regressions and manufactured position/velocity/mass
agreement boundaries. Full suite: 3,567 pass in 628.94 s; Ruff, strict OpenSpec,
diff whitespace checks and unchanged legacy SHA-256 pass. D4.3 still needs
executor-level replay verification; no native research experiment has run.
...
D4.3 implementation (2026-09-24, based on `e9f125c`): nominal completion gates
one tighter replay in a fresh, identity-matched environment under the original
budget. 124 focused checks pass in 2.08 s, including real native nominal/tighter
toy propagation, six-launch accounting, unchanged commands/inputs, threshold
agreement/disagreement and preparation/native/deadline failures. Existing report
tests cover position, velocity and mass thresholds and absent-value semantics.
Full suite: 3,598 pass in 626.61 s; Ruff, strict OpenSpec, diff whitespace and
unchanged legacy SHA-256 pass. The toy test does not qualify the
full-force mission; D4.4 and strict M3 gates remain open.
...
- [x] 0.1 Reuse existing CLI-compatible runtime/reference/model/kernel metadata and capture a detached normalized scenario snapshot without a fictitious file identity; verify shared-field parity, edited dry_mass_kg=500, JSON compatibility, snapshot detachment and unchanged CLI payloads/lazy imports.
- [x] 0.2 Build result and provenance under SCIENCE_LOCK and publish them together; add the read-only "О расчёте" panel and extend invalidation; verify visible metadata/conventions/counts, edits/reload cleanup, search/resource/sampling failure cleanup, and zero additional search or provenance calls on slider/selection changes.
- [x] 0.3 Run focused UI/explorer/CLI checks, Ruff, the complete pinned test suite, legacy checksum/import checks and strict OpenSpec validation; verify all pass with unchanged scientific tolerances, then update current state and roadmap without marking unfinished M3 tasks complete.

D3 evidence (2026-09-23, implementation based on `6a80424`): 29 focused
explorer/UI/CLI checks pass in 9.71 s; full suite: 3,502 pass in 608.17 s.
Ruff and strict validation pass. Legacy SHA-256 remains
`4f0bb03da0eef3a7b91f63bb2b1e5906554379b2f767797fa2f32a5289b7fedf`;
no source import/reference to the legacy module was found. AppTest verifies the
panel; browser visual QA was not performed. The initial UI test counted unrelated
JSON blocks and failed; it was corrected to inspect the named panel directly.
No scientific tolerances changed. Remaining M3 tasks are still incomplete.
...
with bounded intervals through degree 8, covering all 5,760 dimensionless
...
a prefix from tail cost alone. Next add exact analytic controls for a
...
and positive whole-chord floors. Existing Jacobian variation bounds apply;
...
incoming-ball closure and identical coverage/resource, without new arcs.
Stage latching and completed output alone cannot establish continuous
safety. Documentation audit only; strict OpenSpec/whitespace pass, no
full-suite rerun. Native rejection/composition remains open, as does3.9.
...
invalid inputs and deadlines. Pinned fresh state/configuration/illumination
checks pass without native calls. Next qualify the Sun velocity binding for
Schwarzschild; PCK/native arithmetic and state/interval domains remain open.
Full verification: 2990 passed in 474.67 s; native inventory 151.39 s,
portable 53.84 s, unchanged thirteen/zero arcs. Retained SRP vector and
5.890205713199613e-24 m/s^2 L2 arithmetic allowance match captured output.
...
rejection cases pass in 0.50 s, including an anisotropic missing-factor
counterexample. This is not fresh Moon/Mars application: next bind the full
arrays, positions, matrices and allowances without new native calls.
Full verification: 2923 passed in 457.16 s; native inventory 141.40 s,
portable 45.28 s, unchanged thirteen/zero arcs. Ruff, strict OpenSpec and
legacy isolation pass. No production or tolerance changes; task3.9 stays open.
