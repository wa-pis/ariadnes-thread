...
def test_stored_mars_degree20_profile() -> None:
    """Replay pinned SI/J2000 inputs without ephemeris queries or propagation."""
    started_s = perf_counter()
    budget = trajectory._RefinementBudget("isolated-mars-degree20-profile", 300.0)
    ...
    args = (budget, field.gravitational_parameter, field.reference_radius, cosine, sine,
            ...
            np.asarray(recorded["inertial_to_fixed"]), 20)
    ...
    assert outward == [[-3.1644671545870344, -3.1352173491822377],
    ...
    print(json.dumps({"isolated_mars_degree20_profile": {
        "snapshot_sha256": sha256(snapshot_bytes).hexdigest(), "prefix_degree": 20,
